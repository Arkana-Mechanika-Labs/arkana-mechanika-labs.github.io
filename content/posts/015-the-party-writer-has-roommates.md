...
The final recovered bytes show `15DF:0DF9` near-calling `15DF:275F`, then `15DF:0DFF` unconditionally jumping to `15DF:275B`, a continuation beyond the current dump window. The add-to-party path does not end at the writer; it hands off into more overlay code that the current `0000-0EFF` dump does not cover.
